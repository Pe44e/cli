...
class Profile extends BaseCommand {
  ...
  async enable2fa (args) {
    ...
    const existingInfo = await get(conf)

    if (!existingInfo.tfa.pending && existingInfo.tfa.mode === mode) {
    ...
    if (userInfo && userInfo.tfa && userInfo.tfa.pending) {
      ...
      await otplease(this.npm, conf, c => set(({ tfa: { password, mode: 'disable' } }, conf)))
    }
